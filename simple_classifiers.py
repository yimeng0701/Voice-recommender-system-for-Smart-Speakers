...
def plot_word_cloud_for_category(cleaned_text, labels, category):
    """plot wordcloud for a certain category"""
    text = ''
    for i in range(len(labels)):
        if category in labels[i]:
            text = text +" "+cleaned_text[i]
...
def encoding(context, methods='TF-IDF'):
    if methods == 'TF-IDF':
        ...
        ds_features= vectorizer.fit_transform(context)  #corpus: context      
...
if __name__ == "__main__":
    ...
    plot_word_cloud_for_category(context,multi_hot_labels,"Basketball")
   
    ds_features,voc = encoding(context,methods="TF-IDF")
    ds_features, labels = shuffle(ds_features,labels)
    #save the label_list
    #with open('../data/voc.p', 'w') as fp:        
    joblib.dump(voc, './data/voc.p')
